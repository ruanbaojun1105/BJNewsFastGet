...
//        String url = String.format(YK_SEARCH_KEYWORD_URL, keyWord);
    ...
    public static Response getWithHeader(String url) {
        Request<String> requestBuild= NoHttp.createStringRequest(url);
        ...
        return SyncRequestExecutor.INSTANCE.execute(requestBuild);
    }
    /**
     * 优酷播放清单中的播放列表 关键字
     *
    ...
    private static boolean isNetAvailable() {
        if (!isNetworkAvailable()) {
            Toast.makeText(App.getInstance(), "网络连接异常", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
    ...
    public static <T> void sendRequest(Request<T> request, OnResponseListener listener){
        AsyncRequestExecutor.INSTANCE.execute(0, request, listener);
    }
}
